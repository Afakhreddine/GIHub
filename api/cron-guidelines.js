...
const INIT_PROMPTS = {
  ...
  AASLD: `Fetch https://www.aasld.org/practice-guidelines and extract ALL clinical practice guidelines listed by disease topic, published from 2000 to present. For each guideline, search PubMed (https://pubmed.ncbi.nlm.nih.gov/?term={title}+AASLD+Hepatology) to find its PMID, then use https://pubmed.ncbi.nlm.nih.gov/{PMID}/ as the url field. Confirm the publication year from PubMed. Return ONLY a JSON array. Each item: ${JSON_SCHEMA}`,
  ...
  ACG:   `Fetch https://gi.org/guidelines and extract ALL ACG clinical guidelines and clinical practice updates published from 2000 to present. For EVERY guideline found, search PubMed at https://pubmed.ncbi.nlm.nih.gov/?term={title}+ACG+%22American+Journal+of+Gastroenterology%22 to find its PMID, then use https://pubmed.ncbi.nlm.nih.gov/{PMID}/ as the url field. If no match with that query, retry with just the guideline title. Confirm the publication year from PubMed. Return ONLY a JSON array. Each item: ${JSON_SCHEMA}`,
};
...
const UPDATE_PROMPTS = {
  ...
  AASLD: `Fetch https://www.aasld.org/news and identify any new or updated AASLD practice guidelines in the past 7 days. For each found, search PubMed to find its PMID and use https://pubmed.ncbi.nlm.nih.gov/{PMID}/ as the url field. Return ONLY a JSON array of new items ([] if none). Each item: ${JSON_SCHEMA}`,
  ...
  ACG:   `Check https://gi.org/guidelines for new ACG guidelines in the past 7 days. For each new guideline found, search PubMed at https://pubmed.ncbi.nlm.nih.gov/?term={title}+ACG+%22American+Journal+of+Gastroenterology%22 to find its PMID and use https://pubmed.ncbi.nlm.nih.gov/{PMID}/ as the url field. Return ONLY a JSON array ([] if none). Each item: ${JSON_SCHEMA}`,
};
