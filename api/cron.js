// Runs once daily at 6am UTC via Vercel Cron
// Fetches all 3 sections via Claude web search and stores in Upstash Redis
...
async function fetchSection(section, apiKey) {
  ...
  if (!response.ok) throw new Error(`Anthropic error: ${JSON.stringify(data?.error)}`);
  const text = (data.content || []).filter(b => b.type === "text").map(b => b.text).join("");
  const match = text.match(/\[[\s\S]*\]/);
  if (!match) throw new Error(`No JSON array in response for ${section}: ${text.slice(0,200)}`);
...
async function redisSet(key, value) {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify(JSON.stringify(value)), // double stringify for Redis string storage
  });
