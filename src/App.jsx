...
function Badge({ label, color }) {
  return (
    <span style={{ display:"inline-block", padding:"2px 10px", borderRadius:20, fontSize:11, fontWeight:700, color:"#fff", background:color||"#334", whiteSpace:"nowrap" }}>
      {label}
    </span>
  );
}
...
function ContentCard({ item, type }) {
  ...
  return (
    ...
    >
      ...
      {type === "articles" && item.authors && (
        <div style={{ fontSize:11.5, color:"#445570", fontStyle:"italic" }}>{item.authors}</div>
      )}
...
function ContentSection({ type }) {
  ...
  const refresh = useCallback(async () => {
    if (sessionCache[type]) {
      setItems(sessionCache[type].items);
      setLastUpdated(sessionCache[type].time);
      setAiActive(true);
      return;
    }
    setLoading(true);
    try {
      const raw = await callClaude(
        PROMPTS[type],
        "You are a medical information curator. Search the web for current information and return only a valid JSON array. Output ONLY the JSON array starting with [ and ending with ]. No markdown, no backticks, no preamble.",
        true
      );
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        const time = new Date();
        sessionCache[type] = { items: parsed, time };
        setItems(parsed);
        setLastUpdated(time);
        setAiActive(true);
      }
    } catch (e) {
      console.error("Content fetch failed, using static:", e.message);
    }
    setLoading(false);
  }, [type]);
    ...
    <div>
      ...
      {search && (
        <div style={{ marginBottom:14, fontSize:12, color:"#3a5878", fontFamily:"monospace" }}>
          ...
          <button onClick={() => setSearch("")} style={{ background:"none", border:"none", color:"#5b8af0", fontSize:12, cursor:"pointer" }}>✕</button>
        </div>
      )}
      {filtered.length === 0
    ...
    >
      ...
      <div style={{ padding:"16px 20px 18px" }}>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
          {edu.features.map(f => (
            <span key={f} style={{ fontSize:11, padding:"3px 10px", borderRadius:20, background:edu.color+"22", color:edu.color, border:`1px solid ${edu.color}44`, fontWeight:600 }}>{f}</span>
          ))}
        </div>
...
function QuizSection() {
  ...
  async function generateQuestion(topicId) {
    ...
    try {
      const raw = await callClaude(
        prompt,
        "You are a gastroenterology board exam question writer. Output ONLY a valid JSON array starting with [ and ending with ]. No markdown, no backticks, no preamble.",
        false
      );
      let parsed = null;
      ...
      if (!parsed) { const m = raw.match(/\{[\s\S]*\}/); if (m) try { parsed = [JSON.parse(m[0])]; } catch {} }
      if (!parsed) { const clean = raw.replace(/```json|```/gi, "").trim(); try { parsed = JSON.parse(clean); } catch {} }
      if (Array.isArray(parsed) && parsed.length > 0) {
        setQuestions(parsed);
      } else {
        setQuizError(`Could not parse response. Raw: ${raw.slice(0, 200)}`);
      }
    } catch (e) {
  ...
  const avgScore = history.length
    ? (history.reduce((a, h) => a + (h.score / h.total * 100), 0) / history.length).toFixed(0)
    : null;
    ...
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:6, flexWrap:"wrap", gap:12 }}>
        <div>
          ...
          <p style={{ fontSize:12.5, color:"#2e4060", marginTop:4 }}>AI-generated board-style MCQs · Fresh question every session · Powered by Claude</p>
        </div>
        {avgScore && (
      ...
      {loading && (
        <div style={{ textAlign:"center", padding:"60px 0" }}>
          ...
          <div style={{ marginTop:6, fontSize:12, color:"#2a3a50" }}>Claude is crafting a clinical vignette based on current guidelines</div>
        </div>
      )}

      {submitted && score !== null && (
        <div style={{ background:score===1?"rgba(76,175,61,0.1)":"rgba(224,82,82,0.1)", border:`1px solid ${score===1?"#4caf7d":"#e05252"}55`, borderRadius:12, padding:"16px 22px", marginBottom:24, display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:10 }}>
          <div>
            <div style={{ fontSize:18, fontWeight:700, color:score===1?"#4caf7d":"#e05252" }}>{score===1?"Correct! 🎉":"Incorrect 📚"}</div>
            <div style={{ fontSize:12, color:"#3a5070", marginTop:3 }}>Review the explanation below. Click any topic for a new question.</div>
          </div>
          <button onClick={() => generateQuestion(sel)} style={{ background:"rgba(91,138,240,0.12)", border:"1px solid rgba(91,138,240,0.3)", color:"#8aafff", padding:"8px 16px", borderRadius:8, fontSize:13, fontWeight:600, cursor:"pointer" }}>New Question →</button>
        </div>
      )}
              ...
              <div key={qi} style={{ background:"rgba(255,255,255,0.025)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:12, padding:"20px 22px" }}>
                ...
                {submitted && (
                  <div style={{ marginTop:12, paddingLeft:38 }}>
                    <div style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:8, padding:"10px 14px" }}>
                      ...
                      <div style={{ fontSize:13, color:"#6a8aaa", lineHeight:1.7 }}>{q.explanation}</div>
                    </div>
                  </div>
                )}
              </div>
      ...
      {!loading && !sel && (
        <div style={{ textAlign:"center", padding:"60px 0" }}>
          ...
          <div style={{ fontSize:15, color:"#2a3a50" }}>Select a topic above to generate a fresh AI question</div>
        </div>
      )}
      ...
      {history.length > 0 && (
        <div style={{ marginTop:48 }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16, flexWrap:"wrap", gap:8 }}>
            ...
            <button
              onClick={() => { setHistory([]); try { localStorage.removeItem("gihub_quiz_history"); } catch {} }}
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))", gap:10 }}>
            {history.map((h, i) => {
              const col = h.score === 1 ? "#4caf7d" : "#e05252";
              return (
                <div key={i} style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:10, padding:"12px 14px", display:"flex", flexDirection:"column", gap:4 }}>
                  ...
                  <div style={{ fontSize:18, fontWeight:700, color:col, marginTop:4 }}>{h.score === 1 ? "✓ Correct" : "✗ Incorrect"}</div>
                </div>
              );
            })}
          </div>
